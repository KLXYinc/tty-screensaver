...
impl ClocksVisualizer {
    ...
    fn project_3d(&self, p: Vec3) -> Option<(i32, i32)> {
        let distance = 5.0;
        let adjusted_z = distance + p.z;
        if adjusted_z <= 0.1 {
            return None;
        }
        let z_factor = (distance / adjusted_z) * self.zoom;
        let proj_x = p.x * z_factor;
        let proj_y = p.y * z_factor;
    ...
    fn project_2d(&self, p: (f64, f64)) -> (i32, i32) {
        let cx = self.width as f64 / 2.0;
        let cy = self.height as f64 / 2.0;
        let zx = cx + (p.0 - cx) * self.zoom;
        let zy = cy + (p.1 - cy) * self.zoom;
        (zx.round() as i32, zy.round() as i32)
    }
    fn draw_line_3d(&self, buffer: &mut ScreenBuffer, p1: Vec3, p2: Vec3, color: Color, ch: char) {
        if let (Some((x0, y0)), Some((x1, y1))) = (self.project_3d(p1), self.project_3d(p2)) {
            buffer.draw_line(x0, y0, x1, y1, ch, color, Color::Reset);
        }
    }
    fn draw_line_2d(
        &self,
        ...
        let (x0, y0) = self.project_2d(p1);
        let (x1, y1) = self.project_2d(p2);
        buffer.draw_line(x0, y0, x1, y1, ch, color, Color::Reset);
    }
}
    ...
    fn draw(&mut self, buffer: &mut ScreenBuffer) {
        ...
        match CLOCK_TYPES[self.clock_idx] {
                        ...
                        for (dx, ch) in row.chars().enumerate() {
                            if ch == '#' {
                                ...
                                for lz in -1..=1 {
                                    ...
                                    let y = ly * 0.8;
                                    let z = lz as f64 * 0.8;
                                    let rotated = Vec3::new(x, y, z)
                                        .rotate_y(-rot_y)
                                        .rotate_x(rot_x)
                                        .rotate_z(rot_z);
                                    let distance = 30.0;
                                    let z_factor = (distance / (distance + rotated.z)) * self.zoom;
                                    if z_factor < 0.0 {
                                        continue;
                                    }
                                    let proj_x = rotated.x * z_factor;
                                    let proj_y = rotated.y * z_factor;
                                    let screen_x =
            ...
            ClockType::Spinning3D => {
                ...
                let rotate = |x: f64, y: f64, z: f64| -> Vec3 {
                    Vec3::new(x, y, z).rotate_x(rot_x).rotate_y(rot_y)
                };
                let radius = 2.0;
                let segments = 60;
                for i in 0..segments {

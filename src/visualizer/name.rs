...
use crate::utils::font::{FontStyle, generate_ascii_art};
...
impl NameVisualizer {
    pub fn new(width: u16, height: u16, palette: ThemePalette, charset: CharSet) -> Self {
        ...
        let name = config.user_name.unwrap_or_else(|| "User".to_string());
        let font_style = FontStyle::Standard;
        let name_art = generate_ascii_art(&name, font_style);
        Self {
            width,
            height,
            palette,
...
impl Visualizer for NameVisualizer {
    fn update(&mut self, delta_time: f64) {
        ...
        let art_w = if self.name_art.is_empty() {
            1.0
        } else {
            self.name_art[0].len() as f64
        };
        let art_h = self.name_art.len() as f64;
        let scale_x = 2.0;
        let scale_y = 2.0;
    ...
    fn draw(&mut self, buffer: &mut ScreenBuffer) {
        ...
        match MODES[self.mode_idx] {
            NameMode::Cylinder => {
                ...
                let repeat = 3;
                for rep in 0..repeat {
                    for (y, row) in self.name_art.iter().enumerate() {
    ...
    fn on_scroll_ext(&mut self, delta: i32, is_ctrl: bool) {
        if is_ctrl {
            let styles = [FontStyle::Standard, FontStyle::Slanted, FontStyle::Mini];
            let current_idx = styles
                .iter()
                .position(|&s| s == self.font_style)
                .unwrap_or(0);
            ...
            self.font_style = styles[next_idx];
            self.name_art = generate_ascii_art(&self.name, self.font_style);
            self.bounce_x = self.bounce_x.clamp(0.0, self.width as f64 - 10.0);
            self.bounce_y = self.bounce_y.clamp(0.0, self.height as f64 - 10.0);
        } else {

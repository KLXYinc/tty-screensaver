...
impl Hex3DVisualizer {
    ...
    fn project(&self, p: Point3D) -> Option<(i32, i32)> {
        let tx = p.x - self.cam_x;
        let ty = p.y - self.cam_y;
        let tz = p.z - self.cam_z;

        let x1 = tx * self.yaw.cos() - ty * self.yaw.sin();
        let y1 = tx * self.yaw.sin() + ty * self.yaw.cos();
        let z1 = tz;
        ...
        let x3 = x2 * self.roll.cos() - z2 * self.roll.sin();
        let z3 = x2 * self.roll.sin() + z2 * self.roll.cos();
        let y3 = y2;
        ...
        let screen_x = (self.width as f64 / 2.0 + (x3 / y3) * fov * 2.0).round() as i32;
        let screen_y = (self.height as f64 / 2.0 - (z3 / y3) * fov).round() as i32;
    ...
    fn draw(&mut self, buffer: &mut ScreenBuffer) {
        ...
        for hex in &hexes {
            ...
            for offset in &corners {
                let p = Point3D {
                    x: hex.world_x + offset.0,
                    y: hex.world_y + offset.1,
                    z: hex.world_z,
                };
